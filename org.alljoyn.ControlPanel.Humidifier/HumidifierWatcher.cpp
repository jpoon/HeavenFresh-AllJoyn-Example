...
HumidifierWatcher::HumidifierWatcher(AllJoynBusAttachment^ busAttachment) :
...
HumidifierWatcher::~HumidifierWatcher()
{
    UnregisterFromBus();
}

void HumidifierWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "org.alljoyn.ControlPanel.Humidifier" };
...
void HumidifierWatcher::OnAnnounce(
    _In_ PCSTR name,
    ...
    _In_ const alljoyn_msgarg aboutDataArg)
{
    ...
    if (alljoyn_aboutobjectdescription_hasinterface(objectDescription, "org.alljoyn.ControlPanel.Humidifier"))
    {
        AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
            AllJoynHelpers::MultibyteToPlatformString(name),
            AllJoynHelpers::GetObjectPath(objectDescription, "org.alljoyn.ControlPanel.Humidifier"),
            port);
...
void HumidifierWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<HumidifierWatcher>
        };
...
void HumidifierWatcher::Start()
{
    ...
    int32 result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_HumidifierIntrospectionXml);
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &HumidifierWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void HumidifierWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void HumidifierWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
    AllJoynHelpers::DispatchEvent([=]() {

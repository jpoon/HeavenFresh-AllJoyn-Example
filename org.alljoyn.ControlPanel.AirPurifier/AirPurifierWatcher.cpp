...
AirPurifierWatcher::AirPurifierWatcher(AllJoynBusAttachment^ busAttachment) :
...
AirPurifierWatcher::~AirPurifierWatcher()
{
    UnregisterFromBus();
}

void AirPurifierWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "org.alljoyn.ControlPanel.AirPurifier" };
...
void AirPurifierWatcher::OnAnnounce(
    _In_ PCSTR name,
    ...
    _In_ const alljoyn_msgarg aboutDataArg)
{
    ...
    if (alljoyn_aboutobjectdescription_hasinterface(objectDescription, "org.alljoyn.ControlPanel.AirPurifier"))
    {
        AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
            AllJoynHelpers::MultibyteToPlatformString(name),
            AllJoynHelpers::GetObjectPath(objectDescription, "org.alljoyn.ControlPanel.AirPurifier"),
            port);
...
void AirPurifierWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<AirPurifierWatcher>
        };
...
void AirPurifierWatcher::Start()
{
    ...
    int32 result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_AirPurifierIntrospectionXml);
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &AirPurifierWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void AirPurifierWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void AirPurifierWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
    AllJoynHelpers::DispatchEvent([=]() {
